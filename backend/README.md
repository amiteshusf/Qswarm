...
**UI-facing API (`/api/v1`):** The **QSwarm Web** frontend should call **`/api/v1/...`** (see [Qswarm-UI](https://github.com/amiteshusf/Qswarm-UI) `src/api/schemas.ts`). These routes are a thin BFF on legacy services; legacy paths (**`/automation/sessions`**, **`/repo-connections`**, etc.) remain unchanged. **`GET /api/v1/dashboard`** is normalized for the Web Zod contract (`sessionCounts` record, mapped `recentSessions[].status`, string-safe ids/refs). **`GET /api/v1/repo-connections`** returns a **top-level JSON array** of `repoConnectionSchema` objects (`owner`, `repo`, `authRef`, optional `displayName` / `cloneUrl`, …)—not `{ "repoConnections": [...] }`. **`POST/PATCH /api/v1/repo-connections`** accept the same shapes as the UI (`repoConnectionInputSchema`); `provider: "other"` is stored as **`github`** until the backend adds a generic provider.
