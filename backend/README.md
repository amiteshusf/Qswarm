...
- **Hosted workspace (Sprint 2):** For **`POST /automation/sessions/{id}/start`**, if **`repo_path`** is unset or does not exist on the server, QSwarm can **clone** into a deterministic managed directory (**`QSWARM_WORKSPACE_ROOT`**, default **`/tmp/qswarm/sessions/<session_id>/repo`**) using optional **`repository_connection_id`** on create (or on the start body), **`GITHUB_TOKEN`** (or the connection’s PAT env), and **`session.base_branch`** / connection **`default_branch`**. After clone, a **Node bootstrap** step runs when **`package-lock.json`** or **`package.json`** is present (**`npm ci`** vs **`npm install`**, **`QSWARM_BOOTSTRAP_TIMEOUT_SECONDS`**) before framework scan / plan / execute. **`local_existing`** workspaces skip npm when **`node_modules`** is already populated (**`QSWARM_SKIP_BOOTSTRAP_IF_NODE_MODULES`**, default true); **hosted** materialized workspaces always run npm when a manifest exists. Local development unchanged when **`repo_path`** points at an existing directory. Non-GitHub providers and non-`github.com` URLs return **`repo_clone_source_unresolvable`** until expanded. Successful hosted runs still require the **coding engine CLI** (e.g. Claude/Copilot) to be installed on the host when not using **`stub`**; **Playwright browser binaries** are not installed by QSwarm in this step and may still need host setup.
