...
**UI-facing API (`/api/v1`):** The **QSwarm Web** frontend should call **`/api/v1/...`** only for browser UX. The stable contract is documented in **`docs/UI_V1_CONTRACT.md`** and enforced by **`pytest -m contract`** (see `tests/test_ui_v1_contract_qswarm_ui.py`). Source-of-truth Zod schemas: [Qswarm-UI `src/api/schemas.ts`](https://github.com/amiteshusf/Qswarm-UI/blob/main/src/api/schemas.ts). Legacy paths (**`/automation/sessions`**, **`/repo-connections`**, etc.) stay supported for scripts; they are **not** the UI contract. **`GET /api/v1/dashboard`** uses mixed JSON (underscore `sessionCounts` keys and `recentSessions[].status` enums). **`GET /api/v1/repo-connections`**, **`GET /api/v1/branch-policies`**, and **`GET /api/v1/sessions`** return **top-level JSON arrays** for list endpoints. **`GET /api/v1/settings`** returns the nested **`settingsSchema`** shape. Session create/mutations accept Qswarm-UI field names (e.g. `sourceRef`, `instruction`) where noted in `app/schemas/ui_v1_models.py`.
