...
from app.services.git_workspace_service import GitWorkspaceError, ensure_git_repo
...
def ensure_pr_workspace_ready(
    ...
    *,
    ...
    """
    Return an absolute repo root ready for ``run_session_pr_pipeline``.
    ...
    cur_patch = _current_patch_version(db, session.id)
    ...
    if not files:
        raise SourceControlConfigurationError(
            "Current patch version has no generated_files; cannot rebuild workspace for PR.",
            code="pr_no_current_patch",
        )
    ...
    try:
        apply_generated_patch(repo_root, files)
