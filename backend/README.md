...
**UI-facing API (`/api/v1`):** **`docs/UI_V1_API_CLASSIFICATION.md`** classifies each route as **backend-first** (same payloads as legacy services + `dict_keys_to_camel`) vs **intentional BFF** (dashboard, session aggregates, branch-policy shaping for now). **`docs/UI_V1_CONTRACT.md`** describes the two-tier model. **Repo connections** and **settings** on `/api/v1` are backend-first (list uses **`items`** like `GET /repo-connections`). **Dashboard** and **sessions** remain BFF until Qswarm-UI aligns to backend schemas or OpenAPI-generated types. Legacy routes stay unchanged.
